import{n as e}from"./mobile-BAwNVBNb.js";var t=`0db2b97fda8bdb113cd7fe172fd59810`,n=null;async function r(){if(n)return n;let e=await(await fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${t}`)).json();return n={},e.genres.forEach(e=>n[e.id]=e.name),n}function i(t,n={}){let r=document.getElementById(`search-results`),i=document.getElementById(`movie-details`);if(r.innerHTML=``,i.innerHTML=``,!t||t.length===0){r.innerHTML=`
        <div>
            <p>No movies found.</p>
            ...
            <div class="movie-action-btn">
                ...
                <button class="add-to-favorites" aria-label="Add to Favorites button">
                    <div>
                        <div class="align-svg">
        ...
        `,i.querySelector(`.watch-trailer-btn`).addEventListener(`click`,()=>{window.location.href=`/cinemate-app/src/movies_selected/movies-selected.html?id=${t.id}&title=${encodeURIComponent(t.title.toLowerCase())}&year=${encodeURIComponent(a)}`}),i.querySelector(`.add-to-favorites`).addEventListener(`click`,()=>{e(t,a,s)}),r.appendChild(i)})}async function a(){let e=document.getElementById(`movie-details`);e.innerHTML=``;try{let n=await r(),a=await(await fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${t}`)).json();if(!a.results||a.results.length===0){e.innerHTML=`<p>No movies found.</p>`;return}i(a.results,n)}catch(e){let t=document.getElementById(`movie-details`);t.innerHTML=`<div class="error-message">
            <p>${e} : Failed to load movies. Please try again.</p>
